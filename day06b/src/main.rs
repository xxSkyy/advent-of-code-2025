...
fn main() {
    ...
    for symbol_idx in 0..line_len - 1 {
        ...
        loop {
            let mut checked_idx = symbol_idx;
            let mut found_num = false;
            let mut acc: usize = 0;

            loop {
                let checked_byte = &input[checked_idx as usize + loops];

                if checked_byte == &SPACE || checked_byte == &NEW_LINE {
                    if found_num {
                        break;
                    }

                    checked_idx += line_len;
                    if checked_idx >= line_len * (lines - 1) {
                        break;
                    }
                    continue;
                }
